...
for (let i of products.data) {
    ...
    imgContainer.classList.add("image-container") // <div class="image-container"></div>

    let image = document.createElement("img")
    image.setAttribute("src", i.image) //<img src="path">
...
document.getElementById("search").addEventListener("click", () => {
    // Get the value from the search input and convert it to uppercase
    ...
    productsName.forEach((items, index) => {
        ...
        if (items.innerText.includes(searchInput)){
            // If it does, remove the "hide" class from the corresponding card  
            cards[index].classList.remove("hide")
        } else{
            // If it doesn't, add the "hide" class to the corresponding card
